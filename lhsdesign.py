# -*- coding: utf-8 -*-
"""
Created on Thu Mar 30 09:14:47 2017

@author: BlazzzingSheep
"""
...
def lhs(n, samples=None, criterion=None, iterations=None):
    ...
    Optional
    --------
    samples : int   #Equals P in Matlab lhsdesign
    ...
    if samples is None: #设定取样点数
        samples = n
    
    if criterion is not None:   #设定取样标准
    ...
    else:   #如无取样标准，则设定为普通随机lhs
        H = _lhsclassic(n, samples)

    if criterion is None:   #如无标准则标准为基于中心
        criterion = 'center'
    
    if iterations is None:  #若迭代未设定，则设定迭代次数为5次
        iterations = 5
        
    if H is None:   #根据取样标准选择应用的函数
